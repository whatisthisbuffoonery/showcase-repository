...
static void	set_cone_size(t_object *obj, char *diam_tok, char *height_tok,
	t_rt *rt)
{
	if (!is_numeric_token(diam_tok) || !is_numeric_token(height_tok))
		msg_exit("co: diameter/height: non-numeric value", rt);
	obj->diameter = ft_atof(diam_tok);
	obj->height = ft_atof(height_tok);
	if (obj->diameter <= 0 || obj->height <= 0)
		msg_exit("co: diameter and height must be positive", rt);
	obj->radius = obj->diameter / 2.0;
	obj->radius_sq = obj->radius * obj->radius;
	obj->inv_radius = 1.0 / obj->radius;
	obj->slope = obj->radius / obj->height;
	obj->half_angle = atan(obj->slope);
	obj->cap_check = obj->slope * obj->slope * obj->height * obj->height;
	obj->cos_sq = cos(obj->half_angle) * cos(obj->half_angle);
	//to read later: obj->cos_sq = 1.0 / (1.0 + (slope * slope));
}
...
int	parse_cone(char **tokens, int i, t_rt *rt)
{
	...
	if (!vec_in_range(obj->dir, VECTOR_L, VECTOR_U))
		msg_exit("co: axis vector components must be in [-1,1]", rt);
	else if (vec_dot(obj->dir, obj->dir) < EPSILON_SQ)
		msg_exit("co: nonzero direction needed", rt);
	obj->dir = vec_norm(obj->dir);
	set_cone_size(obj, require_token(tokens, i + 3, rt),
		require_token(tokens, i + 4, rt), rt);
	obj->material.color = parse_color(require_token(tokens, i + 5, rt), rt);
	init_cap(obj, obj->height, rt);
	obj->cone_factor = 1.0f + pow(obj->radius / obj->height, 2.0f);
	return (i + 6);
}
